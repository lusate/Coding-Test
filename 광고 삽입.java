...
class Solution {
	...
	public String solution(String play_time, String adv_time, String[] logs) {
        ...
        //int 범위 벗어남. 그래서 long
        long cur = 0;
        for(int i=0; i<getTime(adv_time); i++){
            cur += totalSec[i];
        }
        //adv_time이 한 칸씩 오른쪽으로 이동하면서 최대값을 구함
        long max = cur;
        int maxIdx = 0; //가장 클 때의 처음 인덱스
        for(int i=getTime(adv_time); i<getTime(play_time); i++){
            cur = cur + totalSec[i] - totalSec[i - getTime(adv_time)];
            if(max < cur){
                max = cur;
                maxIdx = i - getTime(adv_time) + 1;
            }
        }
